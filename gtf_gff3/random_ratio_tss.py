...
def get_ratio_TSS(inside_bed, outside_bed, o_dir, input_bams, k, gtf_type, bam_header_file, verbose = True): 
...
## get the maximum the ratios across replicates and return it as a dictionary
    ...
    merged_dict = {}
    merged_all = pandas.DataFrame(columns=['id'])
    for input_bam in input_bams:
        ...
        merged_dict[input_bam] = merged
    merged_all['id'] = merged_dict[input_bams[0]]['id']
    for input_bam in input_bams:
        merged_all[input_bam] = merged_dict[input_bam]['ratio_TSS']
    merged_all.set_index('id', inplace=True)
    merged_all['max'] = merged_all.max(axis=1)
    #os.system('rm {i} {o}'.format(i=inside_bed, o=outside_bed))
    merged_all.to_csv(out_TSS_tmp, sep = "\t", header=False)
    ...
    title = ['INSIDE_CHROM', 'INSIDE_START', 'INSIDE_END', 'INSIDE_ID', 'INSIDE_SCORE', 'INSIDE_STRAND',
    'OUTSIDE_CHROM', 'OUTSIDE_START', 'OUTSIDE_END', 'OUTSIDE_ID', 'OUTSIDE_SCORE', 'OUTSIDE_STRAND',
    'ID']
    title = title + input_bams + ['MAX_RATIO']
    title = '\t'.join(title) + '\n'
    with open(out_TSS_file, 'r+') as file:
        content = file.read()
        file.seek(0)
        file.write(title + content)


def get_random_tss(input_bams, output_dir, reference, reference_gtf, min_size, random_tss_per_chr, mask, gtf_feature, not_parse_reference, write_reference_Ns, verbose):
    ...
    if verbose:
        print("read BAM header %s" % input_bams[0])
    bam_header_file = get_bam_header2(input_bams[0], output_dir)
    ...
    for k in list(bam_header.keys()):
        ...
        if len(gtf_tss_k) > 0:
            ...
            get_ratio_TSS(tss_inside_bed, tss_outside_bed, output_dir, input_bams, k, "random_tss", bam_header_file, verbose)
        if len(gtf_tes_k) > 0:
            ...
            get_ratio_TSS(tes_inside_bed, tes_outside_bed, output_dir, input_bams, k, "random_tes", bam_header_file, verbose)
        if len(gtf_introns_k) > 0:
            ...
            get_ratio_TSS(introns_inside_bed, introns_outside_bed, output_dir, input_bams, k, "random_introns", bam_header_file, verbose)
        if len(gtf_subset_merge_k) > 0:
            ...
            get_ratio_TSS(feature_inside_bed, feature_outside_bed, output_dir, input_bams, k, "random_feature", bam_header_file, verbose)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_bams", help="Input BAM files", nargs='+')
    ...
    print(args)
    get_random_tss(args.input_bams, args.output_dir, args.reference, args.reference_gtf, args.min_size, args.random_tss_per_chr, args.mask, args.gtf_feature, args.not_parse_reference, args.write_reference_Ns, args.verbose)
