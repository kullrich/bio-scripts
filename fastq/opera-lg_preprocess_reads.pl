...
sub mapWithBowtie
{
    ...
    if( $type eq "cs" )
    {
	$command = "${path}bowtie -v 3 -a -m 1 -S -t -C $fasta -p ${num_cpu} $folder$contigName[ -1 ] - 2>${folder}bowtie.err | sort -n > $folder$outputFile";
    }
    else
    {
	$command = "${path}bowtie -v 3 -a -m 1 -S -t $fasta -p ${num_cpu} $folder$contigName[ -1 ] - 2>${folder}bowtie.err | sort -n > $folder$outputFile";
    }
}
...
sub findSAWithBwa
{
    ...
    $command = "${path}bwa aln -t ${num_cpu} $folder$contigName[ -1 ] - > ${folder}read.sai";
}
...
sub generateAlignmentUsingBwa
{
    ...
    $command = "${path}bwa samse -n 1 $folder$contigName[ -1 ] ${folder}read.sai - | grep '\\(^@\\|XT:A:U\\)' | ${samtoolsDir}samtools view -S -h -b -F 0x4 - | ${samtoolsDir}samtools sort -O SAM -\@ ${num_cpu} -no - > $folder$outputFile";
    print $command."\n";
}
